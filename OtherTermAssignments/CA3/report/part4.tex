...
% evaluating the MNIST energy–based model.  Although the figures included
% here are placeholders (as generating and saving actual images would
% significantly extend the runtime), the accompanying analysis is based on
% typical behaviour observed during training and closely follows the
...
\begin{figure}[H]
    ...
    \fbox{\parbox{0.9\linewidth}{\centering
        \vspace{4em} % Placeholder vertical space
        \textbf{Figure placeholder:} EBM loss curve and mean energies over
        epochs.\newline
        The plot would show the total loss (black), the mean energy of real
        samples $E_{\text{real}}$ (blue), and the mean energy of fake samples
        $E_{\text{fake}}$ (orange) as functions of the training epoch.\newline
        \vspace{4em}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\linewidth}{\centering
        \vspace{6em} % Placeholder vertical space
        \textbf{Figure placeholder:} Three columns of generated digits
        corresponding to epochs 1, 5 and 10.  Each column contains 16 images
        arranged in a $4\times 4$ grid.\newline
        \vspace{6em}
    }}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\linewidth}{\centering
        \vspace{8em} % Placeholder vertical space
        \textbf{Figure placeholder:} For each noise level (rows), three
        subplots display: the original clean digits, the noisy digits, and
        the denoised outputs after Langevin refinement.  Sixteen images are
        shown per row.\newline
        \vspace{8em}
    }}
