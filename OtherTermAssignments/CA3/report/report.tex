...
\subsection{Visualization Gallery and Detailed Analysis}
We now present a comprehensive gallery of generated samples, denoising results and trajectory visualizations, each accompanied by precise analysis. All figures are saved under the project \texttt{images/} folder (paths in captions), so readers can regenerate or inspect outputs directly. For each group we include: (i) representative sample grids, (ii) denoising comparisons across noise levels, (iii) optimization curves, and (iv) sampling trajectories showing the evolution from noise to digits.

\subsubsection{EBM: Samples, Denoising, and Trajectories}
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.95\linewidth]{../images/ebm/ebm_samples_epoch10.png}
    \caption{EBM samples from uniform noise after training (path: \texttt{images/ebm/ebm\_samples\_epoch10.png}). The samples show a range of digit styles; however some strokes are thicker and edges smoother than real MNIST, indicating partial mixing during Langevin refinement.}
    \label{fig:ebm_samples_full}
\end{figure*}

\paragraph{Denoising across noise levels} Figure~\ref{fig:ebm_denoising} shows input noisy images and EBM-denoised outputs for Gaussian noise levels similar to those used in the NCSN experiments. The model successfully removes moderate noise (e.g., $\sigma=0.2$), while stronger noise ($\sigma\ge 0.4$) may yield oversmoothed details or slight mode shifts in stroke thickness.
\begin{figure*}[ht]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\linewidth]{../images/ebm/ebm_noisy_epoch10.png}
        \caption{Noisy inputs (example, \texttt{images/ebm/ebm\_noisy\_epoch10.png})}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\linewidth]{../images/ebm/ebm_denoised_epoch10.png}
        \caption{EBM denoised outputs (example, \texttt{images/ebm/ebm\_denoised\_epoch10.png})}
    \end{subfigure}
    \caption{EBM denoising comparisons.
    The denoised images recover global digit structure; fine detail occasionally degrades under higher noise levels, suggesting room for improved sampler mixing or stronger model capacity.}
    \label{fig:ebm_denoising}
\end{figure*}

\paragraph{Sampling trajectories} To visualize how Langevin dynamics constructs images, we save intermediate sampling frames. Figure~\ref{fig:ebm_traj} presents representative snapshots (frames saved under \texttt{images/ebm\_traj/}). The trajectory illustrates coarse-to-fine refinement: early iterations fill large stroke regions; later iterations add sharper contours. Limited step-count or step-size may explain remaining blurriness.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.95\linewidth]{../images/ebm_traj/trajectory_010.png}
    \caption{Representative EBM trajectory snapshot (frame from \texttt{images/ebm\_traj/trajectory\_010.png}).}
    \label{fig:ebm_traj}
\end{figure}

\subsubsection{NCSN: Samples, Conditional Generation, and Denoising}
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.95\linewidth]{../images/ncsn/samples_epoch30.png}
    \caption{Unconditional NCSN samples after training (path: \texttt{images/ncsn/samples\_epoch30.png}). Samples demonstrate crisp strokes and clear digit identities, reflecting the advantage of multiscale score supervision.}
    \label{fig:ncsn_samples_full}
\end{figure*}

\paragraph{Conditional grid} Figure~\ref{fig:ncsn_cond_full} shows a 10-row grid where each row is conditioned on a label 0--9 (path: \texttt{images/ncsn\_cond/samples\_epoch30.png}). Conditioning yields improved class-specific fidelity and reduces intra-class variation that causes ambiguity in unconditional sampling.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.95\linewidth]{../images/ncsn_cond/samples_epoch30.png}
    \caption{Conditional NCSN grid (path: \texttt{images/ncsn\_cond/samples\_epoch30.png}).}
    \label{fig:ncsn_cond_full}
\end{figure}

\paragraph{Denoising performance across noise levels} NCSN denoising results are plotted in Figure~\ref{fig:ncsn_denoise_all}. At low noise ($\sigma=0.2$) the model nearly recovers original digits; at $\sigma=0.4$ results are still high-quality with some artifacts; at $\sigma=0.6$ digits are recognizable but with thicker strokes. Paths: \texttt{images/ncsn_infer/noisy\_*.png} and \texttt{images/ncsn_infer/denoised\_*.png}.
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.95\linewidth]{../images/ncsn_infer/denoised_0.20.png}
    \caption{Example NCSN denoising at $\sigma=0.2$ (path: \texttt{images/ncsn\_infer/denoised\_0.20.png}).}
    \label{fig:ncsn_denoise_all}
\end{figure*}

\subsubsection{Training Curves and Ablation}
We save training curves for both methods so readers can inspect convergence and stability. Example paths: \texttt{images/ebm/ebm\_loss.png} and \texttt{images/ncsn/ncsn\_loss.png}. These figures illustrate typical behavior: EBM loss shows a narrowing gap between $E_{\text{real}}$ and $E_{\text{fake}}$, while DSM loss decreases steadily across epochs. Ablations (not exhaustive) indicate that:
\begin{itemize}[leftmargin=*]
    \item Increasing Langevin steps improves EBM sample sharpness but increases compute time.
    \item Weighting DSM by $\sigma^2$ stabilizes high-noise learning and improves final sample quality.
\end{itemize}

\subsubsection{What was missing and what we added}
We implemented several missing pieces from the initial codebase to ensure reproducible, richly visualized experiments in Colab:
\begin{enumerate}[leftmargin=*]
    \item Notebook cells to run short interactive demos and full training directly from the notebook (EBM and NCSN). These use \texttt{train\_interactive(...)} helpers in the scripts.
    \item Trajectory sampling helpers that record intermediate frames during Langevin and ALD sampling; these are saved as PNGs and can be converted to GIFs for compact visualization.
    \item Automatic saving of generated grids, denoised examples, and loss curves under standardized paths in \texttt{images/} for direct inclusion in the report.
    \item Visualization utilities in the notebook to assemble GIFs, show saved images, and export figures for the report.
\end{enumerate}

\section{Conclusions and Future Work}
We implemented both EBMs (with Langevin sampling and contrastive divergence) and NCSNs (with weighted DSM and ALD) and evaluated them thoroughly on MNIST. NCSN outperforms EBM in sample sharpness and denoising fidelity, while EBMs remain compelling due to their explicit energy formulation and flexibility for downstream constraints. Future work includes better MCMC mixing strategies (e.g., parallel tempering), larger models for EBMs, and extending NCSN sampling with efficient ODE/SDE solvers.

\section*{Appendix A: How to reproduce figures}
All images referenced in this report are saved under \texttt{next\_year/CA3/images/} with subfolders \texttt{ebm/}, \texttt{ncsn/}, \texttt{ncsn\_cond/}, and \texttt{ncsn\_infer/}. To reproduce these figures in Colab:
\begin{enumerate}[leftmargin=*]
    \item Upload the CA3 folder to Colab and install requirements: \texttt{!pip install -r ../requirements.txt}.
    \item Run the notebook cells in order: setup, install, data, then either demo or full training cells.
    \item Use the provided inference cells (e.g., \texttt{ebm\_infer.generate\_and\_denoise}) to create denoising outputs and trajectories.
    \item GIFs of trajectories are produced by collecting frames and using \texttt{imageio.mimsave} (notebook helper provided).
\end{enumerate}

\section*{Acknowledgements}
We thank the course staff for the clear assignment description and starter code; references are provided above.

\bibliographystyle{IEEEtran}
\bibliography{refs}    \item \textbf{Denoising Robustness:} NCSN denoising is superior at all noise levels, recovering fine details even at $\sigma=0.6$. EBM denoising works well for moderate noise but struggles with high-frequency artifacts.
\end{itemize}
