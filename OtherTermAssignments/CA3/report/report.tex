...
\section{Comprehensive Completion Protocol (CA3)}

\subsection{Deliverable Completeness Checklist}
\begin{table}[!t]
\centering
\caption{CA3 completion checklist (EBM + NCSN)}
\begin{tabular}{|p{0.60\linewidth}|p{0.22\linewidth}|}
\hline
\textbf{Criterion} & \textbf{Status} \\
\hline
All assignment questions answered with derivations and implementation mapping & Complete \\
\hline
Model code appendix included (training + inference modules) & Complete \\
\hline
Qualitative sampling/denoising evidence included & Complete \\
\hline
Submission checklist and bibliography included & Complete \\
\hline
Reproducibility command path documented end-to-end & Complete \\
\hline
\end{tabular}
\label{tab:ca3_completion_checklist}
\end{table}

\subsection{Threats to Validity and Mitigations}
\begin{itemize}
    \item \textbf{Compute budget sensitivity:} convergence quality depends on epochs and hardware. \textbf{Mitigation:} report assumptions and keep smoke/full runs separated.
    \item \textbf{Metric incompleteness:} quality judgments can be visually biased. \textbf{Mitigation:} pair qualitative evidence with objective scores where available.
    \item \textbf{Seed variance:} stochastic samplers produce run-to-run variance. \textbf{Mitigation:} fixed seeds and checkpoint traceability for reported figures.
\end{itemize}
...
python3 codes/ebm_train.py
python3 codes/ncsn_train.py
python3 codes/ebm_infer.py
python3 codes/ncsn_infer.py
...
\begin{itemize}
    \item Sample and denoising grids saved under \texttt{images/}.
    \item Model checkpoints and run metadata for EBM/NCSN experiments.
