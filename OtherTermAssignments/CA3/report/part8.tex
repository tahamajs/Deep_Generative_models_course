...
\texttt{codes/code\_results/} and \texttt{my\_results\_last}. The data loader
surfaces an explicit error message for this condition.

\subsection{Archived Configuration Snapshot}

To ensure this document is decision-complete for a reviewer, we summarize the
exact archived configurations used to generate the integrated figures.

\begin{table}[H]
\centering
\caption{Archived EBM run configuration used in this report.}
\label{tab:ebm-archived-config}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Field} & \textbf{Value} \\
\midrule
Source file & \texttt{codes/code\_results/my\_results\_last/run\_info.json} \\
Epochs & 15 \\
Learning rate & $2\times 10^{-4}$ \\
$\lambda$ regularization & $10^{-4}$ \\
Langevin step size & 0.1 \\
Langevin steps & 60 \\
Langevin noise scale & 0.01 \\
Clamp range & $[-1,1]$ \\
Batch size & 64 \\
Seed & 42 \\
...
\begin{table}[H]
\centering
\caption{Archived NCSN run configuration used in this report.}
\label{tab:ncsn-archived-config}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Field} & \textbf{Value} \\
\midrule
Source file & \texttt{codes/code\_results/my\_results\_last/ncsn/run\_info.json} \\
Epochs & 30 \\
Learning rate & $2\times 10^{-4}$ \\
Sigma range & $\sigma_{\max}=30.0,\ \sigma_{\min}=0.01$ \\
Noise levels & 10 \\
ALD steps per level & 150 \\
ALD base step LR & $2\times 10^{-5}$ \\
Embedding dim & 128 \\
Batch size & 32 \\
Seed & 42 \\
Recorded device & CUDA \\
...
\subsection{Data and Artifact Constraints}

The report is built to be reproducible in constrained environments, with the
following explicit constraints:
\begin{itemize}[leftmargin=*]
  \item \textbf{Offline compatibility:} if MNIST download is unavailable, the
  report can still be rebuilt from archived artifacts.
  \item \textbf{NCSN loss trace availability:} some archived bundles contain
  sample checkpoints but not full per-step loss histories; in that case the
  NCSN loss panel is presented as an explicit fallback notice instead of a
  synthetic curve.
  \item \textbf{Metric scope:} clean-target MAE is computed for EBM denoising
  because matched clean, noisy, and denoised triplets are available in one
  archive. NCSN diagnostics focus on transformation magnitude and texture
  statistics unless matched clean references are present.
\end{itemize}
...
metrics from archived denoising grids. Diagnostics are generated by the report
asset pipeline and stored as report artifacts. Figure~\ref{fig:denoise-metrics}
...
\begin{tabular}{@{}ccccc@{}}
\toprule
\textbf{$\sigma$} & \textbf{\shortstack{MAE\\Noisy-Clean}} &
\textbf{\shortstack{MAE\\Denoised-Clean}} & \textbf{MAE Gain} &
\textbf{\shortstack{Mean Abs\\Update}} \\
\midrule
...
updates across all noise levels. A Laplacian ratio above 1 indicates
high-frequency sharpening relative to the noisy input (observed at
$\sigma=0.2$ and $0.4$), while values near 1 indicate a near-balance between
denoising and smoothing (observed at $\sigma=0.6$).
