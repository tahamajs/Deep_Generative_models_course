...
def dsm_loss(
    model: nn.Module, x: torch.Tensor, cfg: NCSNConfig, sigmas: torch.Tensor, y: Optional[torch.Tensor] = None
) -> torch.Tensor:
    batch_size = x.size(0)
    ...
    loss = 0.5 * (score - target).pow(2).reshape(batch_size, -1).mean(dim=1) * (sigma ** 2)
    return loss.mean()
