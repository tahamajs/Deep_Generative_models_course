...
\subsection{Comprehensive Error Taxonomy}

To make the analysis decision-complete, we decompose failures into three categories:
\begin{itemize}
    \item \textbf{Correct numeric}: a number is produced and equals ground truth.
    \item \textbf{Incorrect numeric}: a number is produced but does not match ground truth.
    \item \textbf{Non-numeric}: no valid number is extractable with regex \texttt{\textbackslash d+}.
\end{itemize}

\begin{table*}[t]
\centering
\caption{Error Taxonomy Decomposition from Generated Metrics}
\label{tab:error_taxonomy}
\begin{tabular}{l|c|c|c|c}
\toprule
Dataset & Model & Correct Numeric & Incorrect Numeric & Non-numeric \\
\midrule
Part 1 (210 samples) & Fine-Tuned & 70 (33.33\%) & 20 (9.52\%) & 120 (57.14\%) \\
Part 1 (210 samples) & Base & 118 (56.19\%) & 92 (43.81\%) & 0 (0.00\%) \\
\midrule
Part 2 (1400 samples) & Fine-Tuned & 411 (29.36\%) & 175 (12.50\%) & 814 (58.14\%) \\
Part 2 (1400 samples) & Base & 730 (52.14\%) & 670 (47.86\%) & 0 (0.00\%) \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t]
\centering
\caption{Pairwise Outcome Decomposition (Fine-Tuned vs Base)}
\label{tab:pairwise_outcomes}
\begin{tabular}{l|c|c|c|c}
\toprule
Dataset & Both Correct & Fine-Tuned Only Correct & Base Only Correct & Both Wrong \\
\midrule
Part 1 (210 samples) & 27 (12.86\%) & 43 (20.48\%) & 91 (43.33\%) & 49 (23.33\%) \\
Part 2 (1400 samples) & 143 (10.21\%) & 268 (19.14\%) & 587 (41.93\%) & 402 (28.71\%) \\
\bottomrule
\end{tabular}
\end{table*}

\textbf{Interpretation of Taxonomy Results}:
\begin{itemize}
    \item The fine-tuned model has stronger \textbf{conditional} numeric accuracy when it emits a number (77.78\% on part 1 and 70.14\% on part 2), but its low numeric coverage (\(\sim\)42\%) dominates overall accuracy.
    \item The base model emits numeric outputs consistently (100\% numeric coverage), which explains its higher overall exact-match accuracy despite lower conditional precision.
    \item The largest actionable gap is therefore \textbf{output-format reliability}, not only counting ability.
\end{itemize}
...
\begin{itemize}
    \item Threat: ROUGE can under-represent semantic correctness for short numeric answers with format variance.
    \item Mitigation: We report both ROUGE and numeric exact-match metrics; we additionally break out coverage vs conditional accuracy.
\end{itemize}

\textbf{Internal validity}:
\begin{itemize}
    \item Threat: Evaluation results depend on a fixed subset split and prior checkpoints.
    \item Mitigation: Deterministic seeds, stable JSON artifacts, and generated tables/plots from explicit input files.
\end{itemize}

\textbf{External validity}:
\begin{itemize}
    \item Threat: CLEVR is synthetic and focused on compositional counting; transfer to natural images is uncertain.
    \item Mitigation: We frame findings as controlled-task evidence and recommend broader VQA benchmarks for deployment claims.
\end{itemize}

\textbf{Reproducibility validity}:
\begin{itemize}
    \item Threat: Manually copied metrics can drift from source outputs.
    \item Mitigation: This report now consumes auto-generated artifacts (\texttt{report/generated/*.tex,json} and \texttt{images/generated/*.png}) via a deterministic script path.
\end{itemize}

\subsection{Reproducibility Protocol (Report-Only)}

For this final report version, we use a report-only regeneration path from committed notebook-result artifacts, with no fresh training/evaluation run required.

\begin{enumerate}
    \item Verify plotting toolchain: \texttt{magick -version}.
    \item Generate assets: \texttt{python3 code/report\_assets/generate\_report\_assets.py --config report/report\_assets\_config.json}.
    \item Validate references: \texttt{python3 code/report\_assets/validate\_report\_assets.py --config report/report\_assets\_config.json}.
    \item Run tests: \texttt{python3 -m unittest discover -s code/report\_assets/tests -p 'test\_*.py'}.
    \item Build report: run \texttt{pdflatex} in \texttt{report/} and confirm figures/tables resolve.
\end{enumerate}

Expected generated outputs:
\begin{itemize}
    \item Quantitative plots: \texttt{rouge\_comparison\_p1.png}, \texttt{rouge\_comparison\_p2.png}, \texttt{accuracy\_comparison\_p1\_p2.png}.
    \item Pattern plots: \texttt{prediction\_pattern\_breakdown\_p1.png}, \texttt{prediction\_pattern\_breakdown\_p2.png}.
    \item Qualitative panel: \texttt{qualitative\_examples\_grid.png}.
    \item Metrics artifacts: \texttt{metrics\_summary.json}, \texttt{metrics\_table.tex}.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Final Report Completeness Checklist}
\label{tab:completeness_checklist}
\begin{tabular}{p{0.54\linewidth}|p{0.18\linewidth}|p{0.20\linewidth}}
\toprule
Criterion & Status & Evidence \\
\midrule
Problem, method, setup, and training configuration documented & Complete & Sections I--IV \\
\midrule
Quantitative metrics included and source-linked & Complete & Generated table + plots \\
\midrule
Qualitative examples and pattern analysis included & Complete & Generated qualitative panel \\
\midrule
Failure analysis and error taxonomy explicit & Complete & Tables \ref{tab:error_taxonomy}--\ref{tab:pairwise_outcomes} \\
\midrule
Threats to validity and limitations discussed & Complete & Validity subsection + conclusion \\
\midrule
Reproducibility workflow and commands documented & Complete & Reproducibility protocol subsection \\
\bottomrule
\end{tabular}
\end{table}
