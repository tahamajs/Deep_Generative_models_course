...
\begin{enumerate}
    ...
    \item Deterministic no-run report reconstruction pipeline for artifact traceability
\end{enumerate}
...
\subsection{Problem Statement and Success Criteria}

The assignment objective is not only to implement score-based generation but also to \textit{demonstrate} correctness and trade-offs in a structured way. Accordingly, we define success criteria in four dimensions:
\begin{itemize}
    \item \textbf{Learning correctness:} score fields should point toward high-density regions of the target distribution.
    \item \textbf{Sampling validity:} generated samples should cover both Gaussian modes with plausible within-mode spread.
    \item \textbf{Method differentiation:} deterministic, Langevin, and annealed samplers should exhibit expected speed/diversity/quality trade-offs.
    \item \textbf{Reproducibility:} report artifacts must be regenerable deterministically from tracked project assets.
\end{itemize}

\subsection{Executive Summary of Outcomes}

The final evidence collected in this report supports four direct conclusions:
\begin{itemize}
    \item \textbf{DSM is effective in this setting:} learned score fields consistently point toward high-density Gaussian modes.
    \item \textbf{Best fixed baseline:} fixed $\sigma = 3$ is the strongest single-scale model among $\sigma \in \{1,3,7\}$.
    \item \textbf{Best overall generation quality:} varying-$\sigma$ training with annealed Langevin sampling gives the most reliable coverage and diversity.
    \item \textbf{Reproducibility is verifiable:} report figures are reconstructed from notebook-embedded outputs with manifest-based traceability and no retraining.
\end{itemize}
...
\toprule
Symbol & Meaning & Role in this report \\
\midrule
$x$ & Data sample in $\mathbb{R}^2$ & Point whose density and score are modeled \\
\midrule
$\sigma$ & Noise scale & Controls smoothing level in DSM and sampler behavior \\
\midrule
$s_\theta(x,\sigma)$ & Learned score function & Neural approximation to $\nabla_x \log p_\sigma(x)$ \\
\midrule
$\epsilon$ & Gaussian perturbation & Corruption noise used in DSM loss construction \\
\midrule
$L(\theta,\sigma)$ & DSM loss & Optimization target during model training \\
\midrule
$x_t$ & Sample at iteration $t$ & State updated at each sampling step \\
\bottomrule
\end{tabular}
\normalsize
\caption{Core notation used consistently across theory and experiments}
\label{tab:notation}
\end{table}
...
\subsection{Experiment Roadmap and Evaluation Protocol}
...
\begin{tabular}{p{0.14\textwidth}p{0.25\textwidth}p{0.25\textwidth}p{0.26\textwidth}}
\toprule
Phase & Objective & Main Outputs & Primary Criteria \\
\midrule
Phase 1 (Fixed $\sigma$) &
Train specialized score models at $\sigma \in \{1,3,7\}$ and isolate noise-scale effects &
Loss curves, score fields, and method-wise sampling panels &
Convergence stability and single-scale sample quality \\
\midrule
Phase 2 (Trajectories) &
Analyze path dynamics from diverse initial points &
Deterministic vs Langevin trajectory visualizations &
Mode reachability and exploration behavior \\
\midrule
Phase 3 (Varying $\sigma$) &
Train one multi-scale model over $\sigma \in [1,20]$ &
Long-horizon training curve and epoch-wise sampling evolution &
Cross-scale robustness and annealed-sampling compatibility \\
\bottomrule
\end{tabular}
\normalsize
\caption{Structured experiment roadmap used throughout this report}
\label{tab:roadmap}
\end{table*}
...
\subsection{Experiment Configuration Snapshot}

\begin{table*}[t]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{0.19\textwidth}p{0.31\textwidth}p{0.44\textwidth}}
\toprule
Category & Configuration & Why it matters \\
\midrule
Dataset & 2D Gaussian mixture, 5000 samples (80/20 split) & Controlled multimodal target for direct visual and convergence validation \\
\midrule
Network & ScoreNet MLP, 4 hidden layers, 256 units each, LeakyReLU + BatchNorm & Balances expressive score estimation with stable optimization \\
\midrule
Fixed-$\sigma$ training & $\sigma \in \{1,3,7\}$, 100 epochs, batch size 256, Adam (lr 0.001) & Isolates per-noise behavior and builds single-scale baselines \\
\midrule
Varying-$\sigma$ training & $\sigma \in [1,20]$ per batch, 1000 epochs, batch size 256, Adam (lr 0.001, step decay) & Enables one model to support multi-scale and annealed sampling \\
\midrule
Samplers & Deterministic, Langevin (50 steps), Annealed Langevin (20 levels, 50 steps/level) & Reveals speed-quality-diversity trade-offs under one setup \\
\bottomrule
\end{tabular}
\normalsize
\caption{Compact view of the experimental configuration used throughout the study}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{output_cell_25_img_3.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{output_cell_25_img_6.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{output_cell_25_img_10.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{output_cell_25_img_16.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{output_cell_25_img_8.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{output_cell_25_img_12.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{output_cell_25_img_19.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{output_cell_27_img_3.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{output_cell_27_img_6.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{output_cell_27_img_9.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{output_cell_27_img_11.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{output_cell_27_img_13.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{output_cell_29_img_2.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_5.png}
        \caption{Epoch 1 - Initial training phase}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_8.png}
        \caption{Epoch 100 - Early training}
    \end{subfigure}
    
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_11.png}
        \caption{Epoch 200 - Mid training}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_14.png}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_17.png}
        \caption{Epoch 400 - Good progress}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_20.png}
        \caption{Epoch 500 - Stable}
    \end{subfigure}
    
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_23.png}
        \caption{Epoch 600 - Refined}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_26.png}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_29.png}
        \caption{Epoch 800 - Maintained quality}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\linewidth}
        \includegraphics[width=\linewidth]{output_cell_29_img_32.png}
        \caption{Epoch 1000 - Near convergence}
    \end{subfigure}
...
\textbf{Trade-offs / Disadvantages:}
\begin{itemize}
...
\subsection{Consolidated Findings Matrix}

To provide a decision-oriented summary, Table~\ref{tab:decision_matrix} consolidates the strongest training and sampling choices for common usage scenarios.

\begin{table*}[t]
\centering
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{0.22\textwidth}p{0.16\textwidth}p{0.17\textwidth}p{0.38\textwidth}}
\toprule
Scenario & Training Strategy & Sampling Strategy & Rationale \\
\midrule
Highest final sample quality &
Varying $\sigma$ [1,20] &
Annealed Langevin &
Best multi-scale robustness and most reliable mode coverage \\
\midrule
Fast prototyping / debugging &
Fixed $\sigma=3$ &
Deterministic &
Short path length and reproducible trajectories with low overhead \\
\midrule
Balanced quality/speed &
Varying $\sigma$ [1,20] &
Standard Langevin &
Good diversity with far fewer updates than annealed sampling \\
\midrule
Specialized single-noise deployment &
Fixed target $\sigma$ &
Langevin at same $\sigma$ &
Maximizes specialization at one operating noise level \\
\bottomrule
\end{tabular}
\normalsize
\caption{Decision matrix for selecting training and sampling configurations}
\label{tab:decision_matrix}
\end{table*}

Across all experiments, the central pattern is consistent: fixed-$\sigma$ models can be excellent specialists, while varying-$\sigma$ training is the most practical default for robust and reusable generation.

\subsection{Research Questions Answered}

Table~\ref{tab:rq_answers} summarizes the primary research questions and their outcomes based on the experiments in this report.

\begin{table*}[t]
\centering
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{0.10\textwidth}p{0.35\textwidth}p{0.47\textwidth}}
\toprule
RQ ID & Question & Answer (from evidence) \\
\midrule
RQ1 & Can DSM with a simple noise-conditioned MLP learn usable score fields on a bimodal 2D target? &
Yes. Score vectors consistently direct samples toward true high-density regions and support stable sampling trajectories. \\
\midrule
RQ2 & Is fixed-$\sigma$ specialization better than varying-$\sigma$ training? &
It is better only at the matched scale; varying-$\sigma$ is more robust and practical overall. \\
\midrule
RQ3 & Which sampler gives the best quality-diversity trade-off? &
Annealed Langevin gives best quality/diversity; standard Langevin is balanced; deterministic is fastest but least diverse. \\
\midrule
RQ4 & Are results reproducible without retraining? &
Yes. Figures are deterministically reconstructed from notebook-embedded outputs and validated against report references. \\
\bottomrule
\end{tabular}
\normalsize
\caption{Research questions and evidence-backed answers}
\label{tab:rq_answers}
\end{table*}

\subsection{Evaluation Criteria and Scoring Protocol}

To make the qualitative comparisons consistent, all method rankings in this report follow a fixed protocol:
\begin{itemize}
    \item \textbf{Quality}: concentration around true modes with low spurious mass.
    \item \textbf{Diversity}: spread of generated points across both modes and within-mode variation.
    \item \textbf{Mode coverage}: percentage of expected modes reached by generated samples.
    \item \textbf{Speed}: relative computational cost measured by number of update steps.
\end{itemize}
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{0.15\textwidth}p{0.22\textwidth}p{0.25\textwidth}p{0.30\textwidth}}
\toprule
Criterion & Evidence Source & Operational Definition & Scoring Rule Used \\
\midrule
Sample quality & Sampling scatter plots and training-stage sample grids & Degree of concentration near high-density regions and absence of obvious off-manifold artifacts & 5-point ordinal scale (higher is better) based on visual concentration and cleanliness \\
\midrule
Diversity & Sampling scatter plots and trajectory spread & Breadth of sample support within and across both Gaussian modes & 5-point ordinal scale using coverage breadth and dispersion patterns \\
\midrule
Mode coverage & Bimodal target structure and generated sample occupancy & Whether both target modes are reached with meaningful occupancy & Reported as approximate percentage plus rank-based comparison \\
\midrule
Speed & Sampler definitions and configured step counts & Relative runtime burden implied by update count & Ranked by step complexity: deterministic $\approx$ Langevin $\ll$ annealed \\
\bottomrule
\end{tabular}
\normalsize
\caption{Standardized evaluation protocol used for cross-method comparisons}
...
\begin{itemize}
    \item Best quality in this study, with the most consistent mode coverage
    \item Excellent diversity and strong refinement in later steps
    \item Slowest runtime due to multi-scale schedule (1000+ updates)
\end{itemize}
...
\textbf{Performance Summary:}
\begin{itemize}
    \item Reliable quality with moderate cost (50 steps)
    \item Stochastic noise improves exploration over deterministic paths
    \item Sensitive to correct noise-level conditioning during use
\end{itemize}
...
\textbf{Performance Summary:}
\begin{itemize}
    \item Fastest method with fully reproducible trajectories
    \item Limited diversity; tends to follow nearest high-density direction
    \item Lower multimodal coverage than stochastic alternatives
\end{itemize}
...
\subsection{No-Run Artifact Reconstruction Workflow}
...
\begin{enumerate}
    \item Extract required \texttt{image/png} outputs from \path{codes/CA3_Score_Based_Models.ipynb}.
    \item Write canonical figure files (\path{output_cell_*}) under \path{images/}.
    \item Generate \path{report/asset_manifest.json} with per-asset SHA-256 hashes and provenance metadata.
    \item Validate every \texttt{\textbackslash includegraphics} reference in \path{report/DGM_CA3_Complete_Report.tex}.
    \item Compile the PDF from LaTeX sources.
\end{enumerate}

This is implemented via:
\begin{itemize}
    \item \path{tools/export_notebook_results.py}
    \item \path{tools/verify_report_assets.py}
    \item \path{tools/build_report_from_notebook_outputs.sh}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Input notebook:} \path{codes/CA3_Score_Based_Models.ipynb}
    \item \textbf{Exported canonical figures:} 35 files matching \path{images/output_cell_*.png}
    \item \textbf{Traceability manifest:} \path{report/asset_manifest.json} with filename, source cell/index, SHA-256, and byte size
    \item \textbf{Reference validation:} every \texttt{\textbackslash includegraphics} in the report must resolve to a present image file
    \item \textbf{Final deliverable:} \path{report/DGM_CA3_Complete_Report.pdf}
\end{itemize}

\subsection{Pipeline Acceptance Criteria and Current Status}

For this repository, completion is defined by explicit acceptance checks rather than subjective inspection. Table~\ref{tab:acceptance_checks} records the required checks and their current outcome under the no-run build workflow.

\begin{table*}[t]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{0.24\textwidth}p{0.17\textwidth}p{0.20\textwidth}p{0.18\textwidth}p{0.13\textwidth}}
\toprule
Check & Expected Target & Observed Result & Validation Source & Status \\
\midrule
Notebook extraction count & 35 canonical images exported & 35 images exported & \path{tools/export_notebook_results.py} output & PASS \\
\midrule
Manifest completeness & 35 entries with hash metadata & 35 entries produced & \path{report/asset_manifest.json} & PASS \\
\midrule
Report reference resolution & All \texttt{\textbackslash includegraphics} paths resolve & 25/25 references resolved & \path{tools/verify_report_assets.py} output & PASS \\
\midrule
Manifest coverage for cited assets & Each cited figure exists in manifest & 25/25 cited assets present & Manifest cross-check output & PASS \\
\midrule
PDF generation & Final report PDF produced & PDF built successfully & \path{report/DGM_CA3_Complete_Report.pdf} & PASS \\
\bottomrule
\end{tabular}
\normalsize
\caption{Objective acceptance criteria used to declare report completeness}
\label{tab:acceptance_checks}
\end{table*}

\subsection{Risk Register and Mitigation Plan}
...
\toprule
Risk & Impact & Trigger Condition & Mitigation in This Project \\
\midrule
Notebook output drift & Figure mismatch with report claims & Embedded notebook outputs changed without synchronized export & Deterministic extractor plus manifest hashing and explicit mapping contract \\
\midrule
Broken figure references & Incomplete PDF or missing evidence & Figure filenames changed or deleted & Automated \texttt{\textbackslash includegraphics} verification before PDF build \\
\midrule
Silent reproducibility regressions & Inconsistent artifact pipeline over time & Script behavior changes without validation & One-command workflow that enforces extraction, verification, and LaTeX build in order \\
\midrule
Over-interpretation of qualitative plots & Invalid or overstated conclusions & Visuals treated as exact quantitative evidence & Explicitly separate qualitative judgments from hard metrics and document limitations \\
\bottomrule
\end{tabular}
\normalsize
\caption{Operational risks and mitigations for artifact-level reproducibility}
...
\begin{itemize}
    \item \textbf{Metric limitation:} The primary quantitative metric is training loss; no FID-style or precision/recall-style metric is reported for the 2D setting.
    \item \textbf{Qualitative bias:} Sampling quality is partly judged from plots (coverage/diversity/trajectory shape), which can introduce observer bias.
    \item \textbf{Stochastic sensitivity:} Results depend on random Gaussian-mixture parameter draws and stochastic training/sampling trajectories.
    \item \textbf{Benchmarking gap:} Runtime trade-offs are comparative but not normalized under a fixed hardware benchmark protocol.
\end{itemize}
...
\subsection{Practical Deployment Checklist}

For practical use beyond this assignment, the following checklist captures the recommended operating path:
\begin{enumerate}
    \item Train a varying-$\sigma$ model first as the default robust baseline.
    \item Validate mode coverage with trajectory and overlap diagnostics before deployment.
    \item Select sampler by objective: annealed (quality), Langevin (balance), deterministic (speed/reproducibility).
    \item Lock seeds and export an asset manifest for report and audit reproducibility.
    \item Re-run evaluation after any architecture or schedule change to prevent silent regressions.
\end{enumerate}

\subsection{Completion Status Summary}

Under the no-run policy used for this project, report completion is defined by reproducibility and evidence integrity, not by rerunning training. The current artifact state satisfies the required completion gates:
\begin{itemize}
    \item Deterministic extraction of notebook-embedded results into canonical report figures.
    \item Manifest-based traceability for exported assets.
    \item Automated verification of all cited figure references.
    \item Successful end-to-end PDF generation from source and validated assets.
\end{itemize}
...
\section{Appendix: Exact Notebook Output Mapping}

Table~\ref{tab:exact_mapping} defines the deterministic mapping from notebook cell outputs to canonical image filenames used by the report pipeline. This mapping is enforced by the export tool and is the source of truth for asset reconstruction.

\begin{table*}[t]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{0.14\textwidth}p{0.15\textwidth}p{0.20\textwidth}p{0.42\textwidth}}
\toprule
Notebook Cell Index & Source Image Indices & Exported Count & Canonical Export Filenames \\
\midrule
11 & 0..1 & 2 & \path{output_cell_9_img_<s>.png}, s in \{0,1\} \\
\midrule
52 & 0..14 & 15 & \path{output_cell_25_img_<s>.png}, s in \{3,6,8,10,12,16,19,21,23,25,29,32,34,36,38\} \\
\midrule
54 & 0..6 & 7 & \path{output_cell_27_img_<s>.png}, s in \{3,6,9,11,13,15,17\} \\
\midrule
56 & 0..10 & 11 & \path{output_cell_29_img_<s>.png}, s in \{2,5,8,11,14,17,20,23,26,29,32\} \\
\bottomrule
\end{tabular}
\normalsize
\caption{Deterministic notebook-output to canonical-filename mapping used by the exporter}
\label{tab:exact_mapping}
\end{table*}

\section{Appendix: Deliverable Inventory}
...
\midrule
\path{tools/export_notebook_results.py} & Script & Deterministic extraction of notebook-embedded \texttt{image/png} outputs to canonical report filenames \\
\midrule
\path{tools/verify_report_assets.py} & Script & Validation that all report figure references resolve and (optionally) are present in manifest \\
\midrule
\path{tools/build_report_from_notebook_outputs.sh} & Script & One-command no-run workflow: extract, verify, then compile PDF \\
\midrule
\path{report/asset_manifest.json} & JSON manifest & Per-asset provenance and integrity data (filename, cell/image index, hash, byte size) \\
\midrule
\path{report/DGM_CA3_Complete_Report.tex} & LaTeX source & Complete structured report with theoretical background, experiments, and reproducibility guarantees \\
\midrule
\path{report/DGM_CA3_Complete_Report.pdf} & Final deliverable & Compiled report artifact generated from no-run workflow \\
\bottomrule
\end{tabular}
\normalsize
\caption{Project deliverables required for complete no-run report reproduction}
\label{tab:deliverable_inventory}
\end{table*}

\section{Appendix: Reproducibility Commands}
\begin{itemize}
    \item \url{source /Users/tahamajs/Documents/uni/venv/bin/activate}
    \item \url{bash tools/build_report_from_notebook_outputs.sh}
\end{itemize}
