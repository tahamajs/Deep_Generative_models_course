...
\begin{enumerate}
    \item Extract required `image/png` outputs from `codes/CA3\_Score\_Based\_Models.ipynb`.
    \item Write canonical figure files (`output\_cell\_*`) under `images/`.
    \item Generate `report/asset\_manifest.json` with per-asset SHA-256 hashes and provenance metadata.
    \item Validate every `\textbackslash includegraphics` reference in `report/DGM\_CA3\_Complete\_Report.tex`.
    \item Compile the PDF from LaTeX sources.
\end{enumerate}

This is implemented via:
\begin{itemize}
    \item `tools/export\_notebook\_results.py`
    \item `tools/verify\_report\_assets.py`
    \item `tools/build\_report\_from\_notebook\_outputs.sh`
\end{itemize}
...
\begin{itemize}
    \item Primary quantitative metric is training loss; no FID-style metric is reported for the 2D setting.
    \item Sampling quality assessment is mostly qualitative (coverage/diversity judged from plots).
    \item Results depend on random Gaussian-mixture parameter draws; broader seed sweeps would improve robustness claims.
    \item Runtime trade-offs are characterized comparatively, not by standardized benchmark hardware timing.
\end{itemize}

These constraints do not invalidate the core findings, but they define the boundary of claims made in this assignment.
...
\begin{itemize}
    \item `source /Users/tahamajs/Documents/uni/venv/bin/activate`
    \item `bash tools/build\_report\_from\_notebook\_outputs.sh`
\end{itemize}
