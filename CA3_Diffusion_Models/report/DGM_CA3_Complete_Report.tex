...
\begin{table}[H]
\centering
\begin{tabular}{p{0.12\textwidth}p{0.30\textwidth}p{0.28\textwidth}p{0.20\textwidth}}
\toprule
...
\begin{figure}[H]
    ...
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \includegraphics[width=\textwidth]{output_cell_29_img_26.png}
...
\begin{figure}[H]
    ...
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \includegraphics[width=\textwidth]{output_cell_29_img_32.png}
...
\begin{table}[H]
\centering
\begin{tabular}{p{0.25\textwidth}p{0.20\textwidth}p{0.20\textwidth}p{0.28\textwidth}}
\toprule
...
\begin{enumerate}
    \item Extract required \texttt{image/png} outputs from \texttt{codes/CA3\_Score\_Based\_Models.ipynb}.
    \item Write canonical figure files (\texttt{output\_cell\_*}) under \texttt{images/}.
    \item Generate \texttt{report/asset\_manifest.json} with per-asset SHA-256 hashes and provenance metadata.
    \item Validate every \texttt{\textbackslash includegraphics} reference in \texttt{report/DGM\_CA3\_Complete\_Report.tex}.
    \item Compile the PDF from LaTeX sources.
\end{enumerate}

This is implemented via:
\begin{itemize}
    \item \texttt{tools/export\_notebook\_results.py}
    \item \texttt{tools/verify\_report\_assets.py}
    \item \texttt{tools/build\_report\_from\_notebook\_outputs.sh}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Input notebook:} \texttt{codes/CA3\_Score\_Based\_Models.ipynb}
    \item \textbf{Exported canonical figures:} 35 files matching \texttt{images/output\_cell\_*.png}
    \item \textbf{Traceability manifest:} \texttt{report/asset\_manifest.json} with filename, source cell/index, SHA-256, and byte size
    ...
    \item \textbf{Final deliverable:} \texttt{report/DGM\_CA3\_Complete\_Report.pdf}
\end{itemize}
...
\begin{itemize}
    \item \texttt{source /Users/tahamajs/Documents/uni/venv/bin/activate}
    \item \texttt{bash tools/build\_report\_from\_notebook\_outputs.sh}
\end{itemize}
