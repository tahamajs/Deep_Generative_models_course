...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="checkpoints/",
        help="Directory of saving checkpoints",
    )
    parser.add_argument(
        "--data_dir", type=str, default="data/", help="Directory of papers.csv"
    )
    parser.add_argument(
        "--log_dir", type=str, default="logs/", help="Directory of putting logs"
    )
...
def main():
    ...
    dataset = NIPS2015Dataset(
        batch_size=config.batch_size, seq_len=config.seq_len, data_folder=args.data_dir
    )
    ...
    checkpoint = torch.load(
        os.path.join(args.checkpoint_dir, "checkpoint.pth"), map_location=device
    )
    ...
    random_lls = [
        rnn.compute_prob(np.asarray([dataset.char2idx[c] for c in text]))
        for text in random_data.values()
    ]
    shakespeare_lls = [
        rnn.compute_prob(np.asarray([dataset.char2idx[c] for c in text]))
        for text in shakespeare_data.values()
    ]
    nips_lls = [
        rnn.compute_prob(np.asarray([dataset.char2idx[c] for c in text]))
        for text in nips_data.values()
    ]
    ...
    for i, snippet in enumerate(snippets):
        ...
        dists = [abs(ll - avg_random), abs(ll - avg_shakespeare), abs(ll - avg_nips)]
